...
const Cultos: React.FC = () => {
  ...
  const renderServiceDetailForm = (isContextual: boolean = false) => (
    <section className={`space-y-8 bg-brand-card ${isContextual ? 'p-8 rounded-3xl border border-brand/20 my-4' : 'p-10 rounded-[48px] border border-white/5 shadow-2xl animate-in slide-in-from-right duration-500'}`}>
       {!isContextual && (
         <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-3xl font-extrabold text-white uppercase tracking-tighter">Adicionar Programação</h2>
              <p className="text-xs text-c-text-secondary font-bold uppercase tracking-widest mt-1">Vincule um horário e igreja a um tipo de atividade</p>
            </div>
         </div>
       )}
       ...
       {!isContextual && (
         <div className="space-y-4">
            <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Tipo de Atividade</label>
            <select
              value={newDetail.eventId}
              onChange={e => setNewDetail({ ...newDetail, eventId: e.target.value })}
              className="w-full bg-brand-bg border border-white/5 rounded-2xl px-8 py-5 text-white font-extrabold outline-none focus:ring-2 focus:ring-brand/30 transition-all appearance-none"
            >
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Título/Tema (Opcional)</label>
          <input
            type="text"
            placeholder="Ex: Culto Dominical"
            value={newDetail.title}
            onChange={e => setNewDetail({ ...newDetail, title: e.target.value })}
            className="w-full bg-brand-bg border border-white/5 rounded-2xl px-8 py-5 text-white font-extrabold outline-none focus:ring-2 focus:ring-brand/30 transition-all"
          />
        </div>

        {!isContextual && (
          <div className="space-y-2">
            <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Nome da Igreja / Instituição</label>
            <input
              type="text"
      ...
      <div className="bg-black/20 p-8 rounded-2xl border border-white/5 space-y-6 relative z-10">
        <h3 className="text-xs font-extrabold text-brand uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="flex gap-2">
              {[
                ...
                <button
                  ...
                  className={`flex-1 py-3 rounded-xl text-[9px] font-extrabold tracking-widest transition-all ${freqType === f.id ? 'bg-brand text-white shadow-lg' : 'bg-white/5 text-c-text-muted'}`}
                >
                  {f.label}
                </button>
              ))}
            ...
            {freqType === 'weekly' && (
              <div className="flex justify-between p-2 bg-brand-bg rounded-2xl border border-white/5 shadow-inner">
                {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
                  <button
                    key={i}
                    onClick={() => setSelectedDays(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])}
                    className={`w-10 h-10 rounded-xl text-[10px] font-extrabold transition-all ${selectedDays.includes(i) ? 'bg-brand text-white shadow-lg' : 'text-c-text-muted hover:text-c-text-secondary'}`}
                  >
                    {d}
                  </button>
                ))}
            ...
            {freqType === 'once' && (
              <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white font-extrabold text-xs outline-none" />
            )}

            {freqType === 'period' && (
              <div className="flex items-center gap-3">
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="flex-1 bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white font-extrabold text-xs outline-none" />
                <span className="text-c-text-muted text-[10px] font-extrabold uppercase">até</span>
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="flex-1 bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white font-extrabold text-xs outline-none" />
              </div>
            )}

            <div className="flex items-center gap-4">
              <input type="time" value={time} onChange={e => setTime(e.target.value)} className="flex-1 bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white font-extrabold text-2xl outline-none" />
              <button
                onClick={handleAddFrequency}
                className="h-full px-6 bg-brand/20 text-brand border border-brand/20 rounded-2xl font-extrabold text-[10px] uppercase hover:bg-brand hover:text-white transition-all"
              >
                Adicionar
              </button>
            </div>
          ...
          <div className="space-y-3 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
            {newDetail.frequencies?.map(f => (
              <div key={f.id} className="flex items-center justify-between bg-white/[0.03] border border-white/5 p-4 rounded-2xl">
                ...
                <button onClick={() => removeFrequency(f.id)} className="text-c-text-muted hover:text-rose-500 transition-colors"><X size={16} /></button>
              </div>
            ))}
            {(!newDetail.frequencies || newDetail.frequencies.length === 0) && (
              <div className="h-full flex flex-col items-center justify-center opacity-20 py-10">
                <Clock size={32} className="mb-2" />
                <span className="text-[10px] font-extrabold uppercase tracking-widest text-center">Nenhum horário definido</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className={`grid grid-cols-1 ${isContextual ? 'md:grid-cols-1 gap-6' : 'md:grid-cols-2 gap-8'} relative z-10`}>
        <div className="space-y-4">
          <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1 flex items-center gap-2">
            <Youtube size={14} className="text-rose-500" /> Link do YouTube / Transmissão
          </label>
          <input type="text" placeholder="https://youtube.com/..." value={newDetail.youtubeUrl} onChange={e => setNewDetail({ ...newDetail, youtubeUrl: e.target.value })} className="w-full bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white text-sm font-medium outline-none" />
        </div>
        <div className="space-y-4">
          <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1 flex items-center gap-2">
            <MapPin size={14} className="text-blue-500" /> Endereço Presencial
          </label>
          <input type="text" placeholder="Rua Exemplo, 123 - Cidade" value={newDetail.address} onChange={e => setNewDetail({ ...newDetail, address: e.target.value })} className="w-full bg-brand-bg border border-white/5 rounded-2xl px-6 py-4 text-white text-sm font-medium outline-none" />
        </div>
      </div>
      ...
      <div className="space-y-4 relative z-10">
        ...
        <textarea
          ...
          className={`w-full ${isContextual ? 'h-32' : 'h-40'} bg-brand-bg border border-white/5 rounded-2xl px-8 py-6 text-white text-sm font-medium outline-none focus:ring-2 focus:ring-brand/30 transition-all resize-none italic`}
        />
      </div>
      ...
      <div className="flex gap-4">
        ...
        {isContextual && (
          <button
            ...
            className="px-8 bg-white/5 text-c-text-secondary font-extrabold rounded-2xl uppercase text-[10px] tracking-widest hover:bg-white/10 hover:text-white transition-all"
          >
            Cancelar
          </button>
        )}
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      <header className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-white/5 pb-8">
        <div>
          <h1 className="text-5xl font-extrabold text-white tracking-tighter uppercase neon-text">Cultos</h1>
          <p className="text-c-text-secondary mt-2 font-medium">Gerencie sua agenda espiritual e momentos de comunhão.</p>
        </div>
        <div className="flex bg-brand-bg p-1.5 rounded-2xl border border-white/5 shadow-inner">
          {[
      ...
      {activeTab === 'list' && !selectedEventId && (
        <div className="space-y-12 animate-in fade-in duration-500">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredEvents.map(event => {
              ...
              return (
                <div
                  ...
                  className="bg-brand-card border border-white/5 rounded-[48px] overflow-hidden hover:border-brand/30 transition-all cursor-pointer group shadow-2xl flex flex-col h-full"
                >
                  <div className="h-48 bg-black/40 relative overflow-hidden shrink-0">
                    ...
                    <div className="absolute top-6 left-6">
                      <span className="bg-brand/80 text-white text-[9px] font-extrabold px-4 py-2 rounded-full uppercase tracking-widest backdrop-blur-md shadow-lg">
                        ATIVIDADE
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-8 flex-1 flex flex-col justify-between gap-6">
                    <div>
                      <h3 className="text-2xl font-extrabold text-white uppercase tracking-tighter group-hover:text-brand transition-colors mb-2 line-clamp-1">{event.title}</h3>
                      <p className="text-[10px] text-c-text-secondary font-bold uppercase tracking-widest">
                    ...
                    <div className="bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-[9px] font-extrabold text-c-text-secondary uppercase tracking-widest">Seu Progresso Hoje</span>
                        <span className="text-lg font-extrabold text-brand tracking-tighter">{progress}%</span>
                      </div>
            ...
            <div className="bg-brand-card border-2 border-dashed border-white/5 rounded-[48px] p-10 flex flex-col items-center justify-center text-c-text-muted hover:border-brand/40 hover:text-brand transition-all group cursor-pointer h-full" onClick={() => setActiveTab('add')}>
              <Plus size={48} className="mb-4 group-hover:scale-110 transition-transform" />
              <span className="font-extrabold text-xs uppercase tracking-[0.2em] text-center">Criar Novo Tipo de Atividade</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500 pb-20">
           ...
           <div className="bg-brand/5 p-8 rounded-[48px] border border-brand/10 flex flex-col md:flex-row gap-8 items-center shadow-2xl relative overflow-hidden">
              ...
              {selectedEvent.thumbnailUrl ? (
                <img src={selectedEvent.thumbnailUrl} className="w-28 h-28 md:w-32 md:h-32 rounded-3xl object-cover border border-white/10 shadow-2xl z-10" />
              ) : (
                <div className="w-28 h-28 md:w-32 md:h-32 rounded-3xl bg-brand-bg border border-white/5 flex items-center justify-center text-brand/30 z-10">
                  <Users size={56} />
                </div>
              )}

              <div className="flex-1 text-center md:text-left z-10">
                 <span className="text-[10px] font-extrabold text-brand uppercase tracking-[0.3em] mb-2 block">TIPO DE ATIVIDADE</span>
                 <h2 className="text-4xl font-extrabold text-white uppercase tracking-tighter">{selectedEvent.title}</h2>
                 <p className="text-c-text-secondary text-xs mt-2 font-bold uppercase tracking-widest bg-white/5 inline-block px-3 py-1 rounded-lg">
                    CRIAÇÃO: {new Date(selectedEvent.createdAt).toLocaleDateString()}
                 </p>
              </div>

              <div className="bg-black/40 p-8 rounded-2xl border border-white/5 text-center min-w-[240px] shadow-inner z-10">
                  <div className="flex justify-between items-center mb-3">
                    <p className="text-[9px] font-extrabold text-c-text-secondary uppercase tracking-widest">Seu Progresso de Hoje</p>
                    <p className="text-2xl font-extrabold text-brand tracking-tighter">{getEventProgress(selectedEventId)}%</p>
                  </div>
           ...
           <div className="flex justify-between items-center px-4">
               <h3 className="text-xs font-extrabold text-c-text-secondary uppercase tracking-widest">Igrejas e Congregações</h3>
               <button
                 onClick={() => {
                 ...
                 className="flex items-center gap-2 px-6 py-3 bg-brand/10 text-brand rounded-full text-[10px] font-extrabold uppercase tracking-widest hover:bg-brand hover:text-white transition-all shadow-lg border border-brand/20"
               >
           ...
           <div className="space-y-6">
              {(() => {
                ...
                return Object.keys(groupedByChurch).map(church => {
                   ...
                   return (
                      <div key={church} className={`border rounded-3xl overflow-hidden transition-all duration-300 shadow-2xl ${isExpanded ? 'bg-brand-card border-brand/20' : 'bg-brand-bg border-white/5 hover:border-white/10'}`}>
                         <div onClick={() => toggleChurch(church)} className="flex items-center justify-between p-8 cursor-pointer hover:bg-white/5 transition-colors group">
                            <div className="flex items-center gap-6 flex-1">
                               <div className={`w-14 h-14 rounded-xl flex items-center justify-center font-extrabold shadow-lg transition-all ${isChurchComplete ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-brand/10 text-brand border border-brand/20'}`}>
                                  ...
                                  {editingChurchName === church ? (
                                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                                      <input
                                        ...
                                        className="bg-brand-bg border border-brand/30 rounded-xl px-4 py-2 text-white font-extrabold uppercase text-xl outline-none"
                                      />
                                      ...
                                      <button onClick={() => setEditingChurchName(null)} className="p-2 bg-white/5 text-c-text-secondary rounded-lg hover:scale-110 transition-all"><X size={16} /></button>
                                    </div>
                                  ) : (
                                    <>
                                       <h3 className={`text-2xl font-extrabold uppercase tracking-tighter transition-colors ${isExpanded ? 'text-white' : 'text-c-text-secondary group-hover:text-white'}`}>{church}</h3>
                                       <p className="text-[10px] text-c-text-secondary uppercase tracking-widest mt-1 font-bold">
                            ...
                            <div className="flex items-center gap-4">
                               <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 p-1.5 rounded-2xl border border-white/5 gap-1">
                                  <button
                                    ...
                                    className="p-3 hover:bg-brand/20 hover:text-brand text-c-text-muted rounded-xl transition-all"
                                  ...
                                  <button
                                    ...
                                    className="p-3 hover:bg-blue-500/20 hover:text-blue-500 text-c-text-muted rounded-xl transition-all"
                                    title="Editar Nome"
                                  >
                                    <Edit2 size={18} />
                                  </button>
                                  <button
                                    ...
                                    className="p-3 hover:bg-rose-500/20 hover:text-rose-500 text-c-text-muted rounded-xl transition-all"
                                    title="Excluir Bloco"
                                  >
                                    <Trash2 size={18} />
                                  </button>
                               </div>
                               <ChevronDown className={`text-c-text-muted transition-transform duration-500 ${isExpanded ? 'rotate-180 text-brand' : ''}`} size={24} />
                            </div>
                         </div>
                           ...
                           <div className="p-8 pt-2 space-y-4 animate-in slide-in-from-top-4 fade-in duration-300">
                              {items.map(detail => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex items-center justify-between p-5 rounded-2xl transition-all group cursor-pointer ${isComplete ? 'bg-white/5 border border-white/5' : 'bg-[#1c232b] border border-white/5 shadow-lg hover:border-brand/40'}`}
                                  >
                                     <div className="flex items-center gap-5 pr-4 flex-1">
                                        ...
                                        <div className="flex-1 min-w-0">
                                           <h4 className={`text-sm font-extrabold uppercase tracking-tight line-clamp-1 ${isComplete ? 'text-c-text-muted line-through' : 'text-white'}`}>{detail.title}</h4>
                              ...
                              {isQuickAddingDetail === church ? (
                                ...
                                <button
                                  ...
                                  className="w-full py-4 border-2 border-dashed border-white/5 rounded-2xl text-[10px] font-extrabold text-c-text-muted uppercase tracking-widest hover:border-brand/40 hover:text-brand transition-all flex items-center justify-center gap-3 group"
                                >
              ...
              {currentEventDetails.length === 0 && (
                <div className="text-center py-24 bg-black/20 rounded-[48px] border border-dashed border-white/5 animate-pulse">
                   ...
                   <h3 className="text-xl font-extrabold text-c-text-muted uppercase tracking-widest">Nenhuma programação definida</h3>
                   <button onClick={() => setActiveTab('add')} className="mt-6 text-brand font-extrabold text-[10px] uppercase tracking-widest border border-brand/20 px-6 py-3 rounded-full hover:bg-brand hover:text-white transition-all">Configurar agora</button>
                </div>
              )}
           </div>
        </div>
      ...
      {activeTab === 'add' && (
        <div className="space-y-12 animate-in slide-in-from-right duration-500">
          ...
          <section className="space-y-8 bg-brand-card p-10 rounded-[48px] border border-white/5 shadow-2xl">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 bg-brand/10 text-brand rounded-2xl flex items-center justify-center border border-brand/20">
                <ImageIcon size={28} />
              </div>
              <div>
                <h2 className="text-3xl font-extrabold text-white uppercase tracking-tighter">Novo Tipo de Atividade</h2>
                <p className="text-xs text-c-text-secondary font-bold uppercase tracking-widest mt-1">Crie um card (ex: Culto Online, Presencial, Devocional)</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Título da Atividade</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Thumbnail (Opcional)</label>
                <input
                  type="text"
                  placeholder="URL da imagem..."
                  value={newEvent.thumbnailUrl}
                  onChange={e => setNewEvent({ ...newEvent, thumbnailUrl: e.target.value })}
                  className="w-full bg-brand-bg border border-white/5 rounded-2xl px-8 py-5 text-white font-medium text-xs outline-none focus:ring-2 focus:ring-brand/30 transition-all"
                />
              </div>
            </div>
           ...
           {selectedEventId && (
             <section className="space-y-6">
               <div className="flex items-center gap-3 text-brand">
                 <Info size={16} />
                 <span className="text-[10px] font-extrabold uppercase tracking-widest">Programações Atuais</span>
               </div>
               <div className="flex flex-col gap-3">
                {currentEventDetails.map(detail => (
                  <div key={detail.id} className="bg-brand-card border border-white/5 p-6 rounded-2xl flex justify-between items-center group">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-c-text-secondary group-hover:text-brand transition-colors">
                        {detail.youtubeUrl ? <Youtube size={20} /> : <Users size={20} />}
                      </div>
                      <div>
                        <h4 className="font-extrabold text-white text-sm uppercase tracking-tight">{detail.title || 'Sem Título'}</h4>
                    ...
                    <button onClick={() => setServiceDetails(prev => prev.filter(d => d.id !== detail.id))} className="w-10 h-10 rounded-xl bg-white/5 text-c-text-muted hover:text-rose-500 hover:bg-rose-500/10 transition-all">
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
      ...
      {activeTab === 'config' && (
        <div className="space-y-12 animate-in slide-in-from-right duration-500">
          <section className="bg-brand-card p-10 rounded-[48px] border border-white/5 shadow-2xl">
            <h2 className="text-3xl font-extrabold text-white uppercase tracking-tighter mb-8">Gerenciar Tipos de Atividade</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Lista de Atividades</label>
                <div className="space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                  {events.map(ev => (
                    <div key={ev.id} className="flex items-center justify-between bg-white/[0.03] border border-white/5 p-5 rounded-2xl group hover:border-brand/40 transition-all">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-brand/10 text-brand rounded-xl flex items-center justify-center font-extrabold">
                           {ev.title.charAt(0)}
                        </div>
                        <span className="text-sm font-extrabold text-white uppercase tracking-tight">{ev.title}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => deleteEvent(ev.id)} className="p-2 text-c-text-secondary hover:text-rose-500 transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="bg-brand/5 border border-brand/20 p-8 rounded-3xl flex flex-col items-center justify-center text-center">
                 <Zap className="text-brand mb-4" size={48} />
                 <h4 className="text-lg font-extrabold text-white uppercase tracking-tighter">Personalize sua Agenda</h4>
                 <p className="text-xs text-c-text-secondary mt-2 font-medium">Edite os cards principais ou adicione novos para organizar diferentes tipos de reuniões e práticas espirituais.</p>
              </div>
            </div>
          </section>
        </div>
      ...
      {activeDetail && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-2 md:p-6 bg-black/95 backdrop-blur-3xl animate-in zoom-in-95 fade-in duration-400">
           <div className="bg-brand-card w-full max-w-4xl h-full md:max-h-[92vh] rounded-[48px] border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden relative">

              {/* Header Modal */}
              <div className="p-8 md:p-10 flex justify-between items-center border-b border-white/5 bg-gradient-to-r from-brand-bg to-brand-card shrink-0">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       <h3 className="text-2xl font-extrabold text-white uppercase tracking-tighter line-clamp-1">{activeDetail.title || 'Momento de Culto'}</h3>
                       <p className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest">{activeDetail.churchNameOrId}</p>
                    </div>
                 </div>
                 <button
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-8 md:p-12 space-y-12">
                 ...
                 {activeDetail.youtubeUrl ? (
                   <div className="bg-brand-bg aspect-video rounded-3xl border border-white/5 overflow-hidden shadow-2xl group relative">
                      <iframe
                   ...
                   <div className="bg-black/20 p-16 rounded-[48px] border border-dashed border-white/5 text-center flex flex-col items-center">
                      <div className="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center mb-6">
                        <Users size={48} className="text-c-text-muted" />
                      </div>
                      <h4 className="text-xl font-extrabold text-c-text-muted uppercase tracking-widest">Acompanhamento Presencial</h4>
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center gap-3 text-brand">
                       ...
                       <h4 className="text-[11px] font-extrabold uppercase tracking-[0.2em]">Suas Anotações e Reflexões</h4>
                    </div>
                    <textarea
                      ...
                      className="w-full h-64 bg-brand-bg/50 border border-white/5 rounded-3xl p-10 text-gray-300 text-lg font-medium leading-relaxed outline-none focus:border-brand/40 transition-all resize-none shadow-inner italic"
                    />
                 </div>
              </div>

              {/* Botão de Conclusão */}
              <div className="p-8 md:p-10 border-t border-white/5 bg-brand-bg/50 shrink-0">
                 <button
                   onClick={() => { toggleServiceCompletion(activeDetail.id, todayStr); setActiveDetailId(null); }}
                   className={`w-full py-8 rounded-2xl font-extrabold uppercase tracking-[0.3em] text-sm flex items-center justify-center gap-4 transition-all shadow-2xl transform active:scale-95 ${activeDetail.completions?.[todayStr] ? 'bg-emerald-500/10 text-emerald-500 border-2 border-emerald-500 shadow-emerald-500/20' : 'bg-emerald-600 text-white hover:bg-emerald-500 hover:translate-y-[-4px] shadow-emerald-600/40'}`}
                 >
