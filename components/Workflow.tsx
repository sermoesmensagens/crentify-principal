    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      <header className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-white/5 pb-8">
        <div>
          ...
          <p className="text-c-text-secondary mt-2 font-medium">Organize suas tarefas diárias e acompanhe seu progresso.</p>
        </div>
        <div className="hidden md:flex gap-2">
          <div className="px-5 py-2.5 bg-brand/10 border border-brand/20 rounded-2xl text-brand font-extrabold text-[10px] tracking-[0.2em] uppercase">
      ...
      <div className={`bg-brand-card border transition-all duration-500 p-10 rounded-[48px] shadow-2xl relative overflow-hidden group ${editingTaskId ? 'border-brand/40 ring-1 ring-brand/20' : 'border-white/5'}`}>
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1">Detalhes da Tarefa</label>
            <input
              type="text"
                ...
                {[
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-[9px] font-extrabold tracking-widest transition-all ${frequency === f.id
                      ...
                      : 'bg-white/5 text-c-text-muted border border-transparent'
                      }`}
                  >
                    {f.label}
                  </button>
            ...
            <div className="bg-black/20 p-6 rounded-2xl border border-white/5 min-h-[80px] flex items-center justify-center">
              {frequency === 'weekly' && (
                ...
                <div className="flex gap-4 items-center w-full">
                  <div className="relative group flex-1">
                    <input ref={startDateRef} type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-brand-bg border border-white/10 p-4 pr-12 rounded-2xl text-[10px] font-extrabold text-brand outline-none focus:ring-1 focus:ring-brand/30 transition-all" />
                    <button 
                  ...
                  <span className="text-c-text-muted font-extrabold text-[10px] uppercase shrink-0">até</span>
                  <div className="relative group flex-1">
                    <input ref={endDateRef} type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-brand-bg border border-white/10 p-4 pr-12 rounded-2xl text-[10px] font-extrabold text-brand outline-none focus:ring-1 focus:ring-brand/30 transition-all" />
                    <button 
               ...
               {(frequency === 'once' || frequency === 'annual' || frequency === 'monthly') && (
                <div className="relative group w-full">
                  <input ref={targetDateRef} type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)} className="w-full bg-brand-bg border border-white/10 p-4 pr-12 rounded-2xl text-[10px] font-extrabold text-brand outline-none focus:ring-1 focus:ring-brand/30 transition-all" />
                  <button 
              ...
              {frequency === 'daily' && (
                <div className="flex items-center gap-2 text-brand/40">
                  <Clock size={16} />
                  <span className="text-[10px] font-extrabold uppercase tracking-widest">Todos os dias sem falta</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-black/20 p-6 rounded-2xl border border-white/5">
            <label className="text-[10px] font-extrabold text-c-text-secondary uppercase tracking-widest ml-1 mb-4 block">Horário da Tarefa</label>
            <div className="flex items-center gap-4">
              ...
              <span className="hidden md:block text-[10px] font-extrabold text-c-text-muted uppercase tracking-widest">Definir Horário</span>
            </div>
          </div>
      ...
      <section className="space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-2">
          <div className="flex items-center gap-3 text-brand">
            ...
            <span className="font-extrabold text-xs uppercase tracking-[0.3em]">Tarefas ({filteredTasks.length})</span>
          </div>
        ...
        <div className="flex flex-col gap-3">
          {filteredTasks.length === 0 ? (
            ...
            filteredTasks.map(task => (
              <div
                key={task.id}
                className={`group flex items-center justify-between p-4 px-6 rounded-2xl border transition-all duration-300 ${task.completions[today]
                  ? 'bg-emerald-500/5 border-emerald-500/20'
                  : editingTaskId === task.id ? 'bg-brand/10 border-brand/40 shadow-[0_0_20px_rgba(108,59,255,0.1)]' : 'bg-brand-card border-white/5 hover:border-brand/30'
                  }`}
              >
                <div className="flex items-center gap-6 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 flex-1 min-w-0">
                    <h3 className={`font-extrabold text-sm tracking-tight uppercase shrink-0 flex items-center gap-2 ${task.completions[today] ? 'text-c-text-muted line-through' : 'text-white'}`}>
                      {task.category}
                    ...
                    <p className={`text-[10px] font-bold uppercase tracking-widest truncate flex-1 ${task.completions[today] ? 'text-gray-800' : 'text-c-text-secondary'}`}>
                    ...
                    <div className="flex items-center gap-2 shrink-0 bg-brand/10 px-3 py-1.5 rounded-xl border border-brand/20">
                      ...
                      <span className="text-[10px] font-extrabold text-brand uppercase tracking-widest">{task.time || '09:00'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 shrink-0">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${editingTaskId === task.id ? 'bg-brand text-white' : 'bg-white/5 text-c-text-secondary hover:text-brand hover:bg-brand/10'
                        }`}
                      title="Editar Tarefa"
                    >
                      <Edit2 size={16} />
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-xl bg-white/5 text-c-text-muted hover:text-rose-500 hover:bg-rose-500/10 transition-all"
