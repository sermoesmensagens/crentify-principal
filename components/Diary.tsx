...
import { Plus, Search, Trash2, Calendar, BookMarked, X, Send, Sparkles, MessageSquare, Quote, Edit2 } from 'lucide-react';
...
const Diary: React.FC = () => {
  ...
  // Added resetForm function to clear the entry state
  const resetForm = () => {
    setEditingId(null);
    ...
    <div className="h-full flex flex-col space-y-10 animate-in fade-in duration-700 pb-24">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-8">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium">Capture momentos de revelação e gratidão.</p>
        </div>
        <button
          // Fixed line 76: Called resetForm() correctly
      ...
      <div className="relative group">
        ...
        <input
          type="text"
          placeholder="Buscar em minhas memórias..."
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border ${entry.type === 'testimony' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                  ...
                  {entry.type === 'testimony' ? 'Testemunho' : entry.type === 'plan' ? 'Plano' : 'Reflexão'}
                </span>
                <button
                  onClick={(e) => deleteEntry(e, entry.id)}
                  className="text-gray-700 hover:text-rose-500 transition-colors p-2"
                >
                  <Trash2 size={16} />
                </button>
              </div>
            ...
            <div className="p-10 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="free">Livre / Reflexão</option>
                    <option value="plan">Plano / Rotina</option>
                    <option value="testimony">Testemunho de Vitória</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex justify-end pt-4">
                ...
                >
                  <Sparkles size={18} />
                  {editingId ? 'ATUALIZAR MEMÓRIA' : 'GUARDAR MEMÓRIA'}
                </button>
              </div>
            </div>
          </div>
