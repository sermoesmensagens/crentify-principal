...
import { ReadingPlan, ReadingPlanCategory, AcademyResource, ReadingPlanContent, Section } from '../types';
import { 
  Sparkles, 
  Search, 
  X, 
...
const ReadingPlans: React.FC<ReadingPlansProps> = () => {
  ...
  const handleBibleNavigation = (resource: AcademyResource, planId: string) => {
    ...
    const match = text.match(/^((?:\d\s)?[a-zA-Záàâãéèêíïóôõöúçñ\s.-]+)\s+(\d+)/i);
  ...
  return (
    <div className=\"relative min-h-screen\">
      {showQuiz ? renderQuiz() : (selectedPlan ? renderSelectedPlan() : renderGallery())}

      {/* MODAL DE LEITURA EMBUTIDO (FIXO E SEMPRE DISPONÍVEL) */}
      {readingResource && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-[#0b0e14]/98 backdrop-blur-3xl" onClick={() => { setReadingResource(null); setIsTimerRunning(false); }}></div>
          
          <div className="bg-[#161b22] w-full max-w-4xl h-full md:max-h-[90vh] rounded-[48px] border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden relative">
            <div className="p-6 md:p-10 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-3xl shrink-0">
              <div className="flex items-center gap-4">
            ...
            <div ref={modalScrollRef} className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-14 space-y-10 focus:outline-none">
              <div className="bg-[#0b0e14]/80 border border-white/5 rounded-[32px] p-6 flex flex-col items-center justify-center text-center shadow-inner relative overflow-hidden max-w-sm mx-auto shrink-0">
                ...
                {(() => {
                  ...
                  if (!chapter) return (
                    <div className="text-center py-20 bg-black/20 rounded-[32px] border border-white/5">
                      ...
                      <p className="text-[10px] text-gray-700 mt-2">Dica: Verifique se os dados da bíblia foram carregados.</p>
                    </div>
                  );
            ...
            <div className="p-6 md:p-8 bg-black/60 backdrop-blur-3xl border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0">
              <div className="hidden md:flex items-center gap-3 text-gray-500 font-black uppercase text-[10px] tracking-widest">
                <Clock size={16} />
                <span>Sugerido: {readingResource.resource.duration || '1 cap'}</span>
              </div>
              <button 
                onClick={() => {
                ...
                className="w-full md:w-auto px-12 py-5 bg-brand text-white rounded-3xl font-black uppercase tracking-[0.2em] text-xs shadow-[0_10px_30px_rgba(var(--brand-rgb),0.3)] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3"
              >
